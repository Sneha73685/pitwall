...
- v9 (M44, `docs/m44-design-review.md`): extended §3a through M43 (M40 track-limits lap exclusion, M41 tyre/stint aggregate exclusion-consistency fix, M42 qualifying Q1/Q2/Q3 results, M43 lap-comparison exclusion warnings) — plus M39's own retroactive entry (in `CHANGELOG.md` and `README.md`), previously missing, following the same pattern M39 itself used for M33. §5's position-history row extended to also credit M40. No scope or architecture change — documentation reconciliation only.
